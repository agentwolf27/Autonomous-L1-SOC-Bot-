...
class AlertTriageClassifier:
    def __init__(self):
        ...
        self.feature_columns = [
            'source_abuse_score', 'dest_abuse_score', 'destination_port',
            'bytes_transferred', 'external_source', 'external_dest',
            'is_internal_traffic'
        ]
        self.categorical_columns = [
            'event_type', 'severity', 'protocol', 'source_whois_country'
        ]
    ...
    def create_training_data(self, df):
        ...
        risk_scores += training_data['source_whois_country'].isin(high_risk_countries) * 20
        ...
        high_risk_events = ['Malware Detection', 'Data Exfiltration', 'Privilege Escalation', 'DDoS Attack']
        medium_risk_events = ['Port Scan', 'Brute Force Attack', 'SQL Injection', 'Cross-Site Scripting']
        ...
        training_data['risk_score'] = np.clip(risk_scores, 0, 100)  # Clip scores to 0-100 range
    ...
    def prepare_features(self, df):
        ...
        for col in self.categorical_columns:
            if col in features_df.columns:
                ...
                try:
                    ...
                    all_values = list(self.label_encoders[col].classes_) + list(unique_values)
        ...
        feature_cols = self.feature_columns + [f'{col}_encoded' for col in self.categorical_columns if col in features_df.columns]
...
def triage(df):
    ...
    triaged_df = triaged_df.sort_values(['priority', 'risk_score'], ascending=[True, False])
...
def get_triage_summary(df):
    ...
    summary = {
        ...
        'critical_alerts': len(df[(df['risk_level'] == 'High') & (df['priority'] == 1)])
    }
    return summary
...
if __name__ == "__main__":
    ...
    print(high_risk[['id', 'event_type', 'source_ip', 'risk_score', 'risk_factors']].to_string())
