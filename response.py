...
class ResponseEngine:
    ...
    def send_email_alert(self, alert):
        ...
        try:
            ...
            self.log_action(
                alert["id"], "email_alert", f"Email notification sent to security team"
            )
            return True
            ...
            self.log_action(
                alert["id"], "email_alert", f"Failed to send email: {e}", "Failed"
            )
            return False
    ...
    def create_ticket(self, alert):
        ...
        try:
            ...
            self.log_action(
                alert["id"], "create_ticket", f"Incident ticket {ticket_id} created"
            )
            return ticket_id
            ...
            self.log_action(
                alert["id"], "create_ticket", f"Failed to create ticket: {e}", "Failed"
            )
            return None
        ...
        try:
            ...
            if source_ip.startswith(
                ...
                self.log_action(
                    alert["id"],
                    "block_ip",
                    f"Skipping block for private IP {source_ip}",
                )
                return False
            ...
            if source_ip in self.blocked_ips:
                self.log_action(
                    alert["id"], "block_ip", f"IP {source_ip} already blocked"
                )
                return True
            ...
            self.log_action(
                alert["id"], "block_ip", f"IP {source_ip} blocked via firewall rule"
            )
            return True
            ...
            self.log_action(
                alert["id"],
                "block_ip",
                f"Failed to block IP {source_ip}: {e}",
                "Failed",
            )
            return False
    ...
    def quarantine_host(self, alert):
        ...
        try:
            ...
            self.log_action(
                alert["id"], "quarantine", f"Failed to quarantine host: {e}", "Failed"
            )
            return False
...
def execute_actions(triaged_df):
    ...
    for idx, alert in response_df.iterrows():
        try:
            ...
            response_df.at[idx, "action_status"] = (
                "Completed" if executed_actions else "Failed"
            )

        except Exception as e:
            logger.error(
                f"Failed to process actions for alert {alert.get('id', idx)}: {e}"
            )
    ...
    logger.info(
        f"Response actions completed. Status summary: {action_summary.to_dict()}"
    )
...
def get_response_summary(df):
    ...
    summary = {
        ...
        "tickets_created": len(
            df[df["actions_taken"].str.contains("create_ticket", na=False)]
        ),
        "emails_sent": len(
            df[df["actions_taken"].str.contains("email_alert", na=False)]
        ),
    }
    return summary
