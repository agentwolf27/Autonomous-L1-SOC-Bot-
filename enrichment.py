...
MITRE_ATTACK_MAPPING = {
    ...
    "Suspicious Network Traffic": ["T1071 - Application Layer Protocol", "T1041 - Exfiltration Over C2 Channel"],
    "Unauthorized Access": ["T1078 - Valid Accounts", "T1021 - Remote Services"],
    "DDoS Attack": ["T1498 - Network Denial of Service", "T1499 - Endpoint Denial of Service"],
    "Data Exfiltration": ["T1041 - Exfiltration Over C2 Channel", "T1048 - Exfiltration Over Alternative Protocol"],
    "Privilege Escalation": ["T1068 - Exploitation for Privilege Escalation", "T1134 - Access Token Manipulation"],
    "SQL Injection": ["T1190 - Exploit Public-Facing Application", "T1059 - Command and Scripting Interpreter"],
    "Cross-Site Scripting": ["T1190 - Exploit Public-Facing Application", "T1203 - Exploitation for Client Execution"],
...
def get_whois_info(ip_address):
    ...
    try:
        ...
        if ip_address in ["8.8.8.8", "1.1.1.1", "208.67.222.222"]:
            return {
                "organization": "Google LLC" if "8.8.8.8" in ip_address else "Cloudflare Inc.",
                "country": "US",
        ...
        if any(pattern in ip_address for pattern in SUSPICIOUS_IP_PATTERNS):
            orgs = ["Anonymous Hosting", "Bulletproof Hosting", "Suspicious Cloud", "Unknown Provider"]
...
def enrich_alerts(df):
    ...
    enrichment_columns = [
        'source_whois_org', 'source_whois_country', 'source_whois_asn',
        'dest_whois_org', 'dest_whois_country', 'source_abuse_score',
        'dest_abuse_score', 'mitre_tags', 'risk_factors', 'is_internal_traffic',
        'external_source', 'external_dest'
    ]
...
if __name__ == "__main__":
    ...
    print(enriched_df[['id', 'source_ip', 'source_abuse_score', 'mitre_tags', 'risk_factors']].head()) 