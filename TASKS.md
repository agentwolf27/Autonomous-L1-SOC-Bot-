...
| 6 | Stop `prepare_features` re-fitting a LabelEncoder on unseen categories, which can shift the codes of known ones; add a test named for unseen_category | claude | `venv/bin/python -m pytest tests -q -k unseen_category` exits 0 |
