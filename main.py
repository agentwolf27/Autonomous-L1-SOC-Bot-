...
try:
    ...
    print("Enterprise integrations not available. Install additional dependencies for Wazuh/Hive integration.")
...
class SOCAutomationBot:
    def __init__(self, config_path: str = "soc_config.json", enterprise_mode: bool = False):
    ...
    def process_alerts_pipeline(self):
        ...
        try:
                ...
                for _, alert in processed_alerts.iterrows():
                    if alert.get('risk_level') in ['High', 'Critical']:
                        try:
                            ...
                            case_id = self.enterprise_integrator.create_case_for_alert(alert_dict)
                            if case_id:
                                processed_alerts.at[alert.name, 'hive_case_id'] = case_id
                                self.logger.info(f"Created The Hive case {case_id} for alert {alert_dict['id']}")
                        except Exception as e:
            ...
            output_file = f"processed_alerts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def print_stats(self):
        ...
        print(f"Total alerts processed: {self.processing_stats['total_alerts_processed']}")
